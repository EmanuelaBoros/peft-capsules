...
from save_activations import (
    # load_activation_probing_dataset,
    load_activation_probing_dataset_args,
)
...
def save_probe_results(args, probe_results, prompt, pca=False):
    ...
    pickle.dump(probe_results, open(os.path.join(save_path, probe_name), "wb"))
...
def load_probe_results(
    ...
    probe_results = pickle.load(open(os.path.join(save_path, probe_name), "rb"))
...
def place_probe_experiment(activations, target, is_test, probe=None, is_lat_lon=True):
    ...
    norm_train_target = (train_target - train_target.mean(axis=0)) / train_target.std(
        axis=0
    )
    ...
    train_pred_unnorm = train_pred * train_target.std(axis=0) + train_target.mean(
        axis=0
    )
    test_pred_unnorm = test_pred * train_target.std(axis=0) + train_target.mean(axis=0)

    projection = probe.predict(activations) * train_target.std(
        axis=0
...
def time_probe_experiment(activations, target, is_test, probe=None):
    ...
    norm_train_target = (train_target - train_target.mean()) / train_target.std()
...
RIDGE_ALPHAS = {
    "Mistral-7B-v0.1": np.logspace(0.8, 4.1, 12),
...
def pca_probe_experiment(args):
    ...
    PCA_DIMS = [2, 4, 6, 8, 10, 15, 20, 30, 40, 50, 75, 100, activations.shape[1]]
